...
#pragma unroll
  ...
  modInv(uint32_t u, uint32_t v)
  {
    ...
    while (u3f > 1.0)
      {
        v2u += u2u * quasiQuoRem<RCP_CAN_BE_HIGH>(v3f, u3f);
        u2u += v2u * quasiQuoRem<RCP_CAN_BE_HIGH>(u3f, v3f);
      }
  ...
  {
    return modMul(u, modInv<RCP_CAN_BE_HIGH>(m.modulus, v), m);
  }
         ...
         uint32_t* __restrict__ moduliList, GmpCudaBarrier bar)
  {
    ...
    myPair.value = (vq == 0) ? MOD_INFINITY : toSigned(modDiv<RCP_CAN_BE_HIGH>(uq, vq, q), q);
        ...
        if (active)
          {
            ...
            if (p > q.modulus)        //  Bring within range.
              p -= q.modulus;
            tq = modDiv<RCP_CAN_BE_HIGH>(modSub(uq, modMul(fromSigned(pair.value, q), vq, q), q), p, q);
            myPair.value = (tq == 0) ? MOD_INFINITY : toSigned(modDiv<RCP_CAN_BE_HIGH>(vq, tq, q), q);
          }
        postMinPair(myPair, bar);
        if (active)
          uq = vq, vq = tq;       
        ...
        if (active)
          {
            ...
            if (pair.modulus > q.modulus)  //  Bring within range.
              p -= q.modulus;
            uq = modDiv<RCP_CAN_BE_HIGH>(modSub(uq, fromSigned(pair.value, q), q), p, q);
