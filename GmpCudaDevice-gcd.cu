...
//  Host only code follows.

int GmpCudaDevice::getGcdKernelOccupancy(void)
{
  int gcdOccupancy;
...
GmpCudaDevice::gcd(mpz_t g, mpz_t u, mpz_t v) throw (std::runtime_error)
{
  ...
  if (deviceSupportsCooperativeLaunch)
  {
    ...
    void* kernel = reinterpret_cast<void *>(gcdKernel);
    assert(cudaSuccess == cudaLaunchCooperativeKernel(kernel, gridSize, GCD_BLOCK_SZ, args));
  }
  else  
    gcdKernel<<<gridSize, GCD_BLOCK_SZ>>>(globalBuf, uSz, vSz, moduliList, *barrier);
